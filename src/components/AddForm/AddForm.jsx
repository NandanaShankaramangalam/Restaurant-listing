...
const AddForm = ({ handleModalOpen, fetchData }) => {
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    const { data } = await api.post("http://localhost:3001/add-restaurant", {
      datas,
    });
    ...
    <div className="fixed top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%] h-96 overflow-scroll w-5/6 md:w-2/6 z-10 bg-yellow-100 shadow shadow-yellow-300 rounded-md">
      <div className="">
        <form className="px-4 mt-5" onSubmit={handleFormSubmit}>
          <div className="flex justify-end" style={{ color: "gray" }}>
            <IoMdClose
              size={32}
          ...
          <div className="mb-4">
            <label for="description" className="block text-gray-600">
              Description
            </label>
            <textarea
              id="description"
              ...
              name="description"
              rows="3"
              className="w-full outline-none resize-none rounded px-3 py-2"
            ></textarea>
          </div>

          <div className="mb-4">
            <label for="address" className="block text-gray-600">
              Address
            </label>
            <input
              type="text"
          ...
          <div className="mb-4">
            <label for="contact" className="block text-gray-600">
              Phone Number
            </label>
            <input
              type="tel"
