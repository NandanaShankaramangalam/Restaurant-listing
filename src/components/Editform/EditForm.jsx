...
const EditForm = ({handleModalOpen, editData, fetchData}) => {

    const [formData, setFormData] = useState({id:editData.id, name:editData.name, description:editData.description, address:editData.address, contact:editData.contact, image:editData.image})
    ...
    const handleFormSubmit = async(e) =>{
        e.preventDefault();
        console.log("forrrmmm=", formData);
        const { data } = await api.patch("http://localhost:3001/edit-restaurant", {
          ...formData
        });
    ...
    <div className='fixed top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%] h-96 overflow-scroll w-5/6 md:w-2/6 z-10 bg-yellow-100 shadow shadow-yellow-200 rounded-md'>
      ...
      <form className='px-4 mt-5' onSubmit={handleFormSubmit}>
        ...
        <div className='text-center py-3'>
            <h1 className='text-xl font-bold'>Edit Restaurant Data</h1>
        </div>
      <div className="mb-4">
        <label for="name" className="block text-gray-600">Restaurant Name</label>
        <input type="text" id="name" value={formData.name} onChange={handleFormChange} name="name" className="w-full border outline-none rounded px-3 py-2"/>
      </div>

      <div className="mb-4">
        <label for="description" className="block text-gray-600">Description</label>
        <textarea id="description" value={formData.description} onChange={handleFormChange} name="description" rows="3" className="w-full outline-none resize-none rounded px-3 py-2"></textarea>
      </div>

      <div className="mb-4">
        <label for="address" className="block text-gray-600">Address</label>
        <input type="text" id="address" value={formData.address} onChange={handleFormChange} name="address" className="w-full outline-none border rounded px-3 py-2"/>
      </div>

      <div className="mb-4">
        <label for="contact" className="block text-gray-600">Phone Number</label>
        <input type="tel" id="contact" value={formData.contact} onChange={handleFormChange} name="contact" className="w-full outline-none border rounded px-3 py-2"/>
      </div>
